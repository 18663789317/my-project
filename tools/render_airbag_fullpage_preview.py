...
def main() -> None:
    ...
    # 顶部 KPI 区
    bar_x, bar_y, bar_w, bar_h = 0.048, 0.785, 0.904, 0.095
    draw_box(ax, bar_x, bar_y, bar_w, bar_h, fc="#10274b", ec="#2a4f79")
    kpi_titles = ["当日净生成(吨)", "当日多/空生成量及均价", "净方向", "当日收盘价"]
    kpi_values = ["-20,000.00", "空 20,000.00 均价 755.00", "空单", "800.00"]
    kpi_colors = ["#ff7b72", "#dbe7f5", "#8ad2ff", "#9fd3ff"]
    col_ratios = [0.20, 0.46, 0.11, 0.23]
    col_edges = [bar_x]
    for r in col_ratios:
    ...
    for i in range(4):
        x0 = col_edges[i]
        if i > 0:
            ax.plot([x0, x0], [bar_y + 0.016, bar_y + bar_h - 0.016], color="#1f3b63", linewidth=1.1)
        text_x = x0 + 0.012 if i == 3 else x0 + (col_edges[i + 1] - x0) / 2
        text_ha = "left" if i == 3 else "center"
        ax.text(text_x, bar_y + bar_h - 0.026, kpi_titles[i], color="#a8bed7", fontsize=12.2, ha=text_ha, va="center")
        value_fs = 18 if i == 1 else 24
        if i == 1:
            center_x = x0 + (col_edges[i + 1] - x0) / 2
            ax.text(center_x, bar_y + 0.028, "空 20,000.00 均价 755.00", color="#67d67d", fontsize=value_fs, weight="bold", ha="center", va="center")
        else:
            ax.text(text_x, bar_y + 0.028, kpi_values[i], color=kpi_colors[i], fontsize=value_fs, weight="bold", ha=text_ha, va="center")
