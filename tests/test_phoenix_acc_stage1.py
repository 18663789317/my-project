...
TARGET_SYMBOLS = {
    # STRATEGY_ALIAS_TO_CODE 现已同时承载凤凰累计和香草期权别名。
    # stage1 仍走 AST 白名单抽取时，若漏掉 VANILLA_OPTION_CODE，
    # 会在 exec 阶段因为字典字面量引用未定义常量而提前失败。
    "VANILLA_OPTION_CODE",
    ...
    "winrate_build_accumulator_runtime_struct",
}
...
class PhoenixAccStage1Tests(unittest.TestCase):
    ...
    def test_boundary_events_tolerate_micro_float_noise(self) -> None:
        call_knock_in = self.simulate([95.0000004], **self.call_terms())
        call_knock_out = self.simulate([109.9999996], **self.call_terms())
        put_knock_in = self.simulate([104.9999996], **self.put_terms())
        put_knock_out = self.simulate([90.0000004], **self.put_terms())
        self.assertEqual(call_knock_in[0]["event_type"], "knock_in_terminate")
        self.assertEqual(call_knock_out[0]["event_type"], "knock_out_subsidy_terminate")
        self.assertEqual(put_knock_in[0]["event_type"], "knock_in_terminate")
        self.assertEqual(put_knock_out[0]["event_type"], "knock_out_subsidy_terminate")
    ...
    def test_winrate_runtime_struct_scales_phoenix_nested_params(self) -> None:
        build_runtime = self.ns["winrate_build_accumulator_runtime_struct"]
        runtime = build_runtime(
            {
                "resolved": {
                    ...
                    "entry_price": 710.0,
                    "strike_price": 830.0,
                    "knock_in_exercise_price": 830.0,
                    "barrier_in": 830.0,
                    "barrier_out": 710.0,
                    "knock_out_price": 710.0,
                    "ko_strike_price": None,
                    "multiple": 2.0,
                    "subsidy_per_ton": 1000.0,
                ...
                "strike_ratio": 830.0 / 710.0,
                "knock_in_exercise_ratio": 830.0 / 710.0,
                "barrier_in_ratio": 830.0 / 710.0,
                "barrier_out_ratio": 710.0 / 710.0,
                "knock_out_ratio": 710.0 / 710.0,
                "ko_strike_ratio": None,
                "multiple": 2.0,
                "subsidy_per_ton": 1000.0,
                "base_qty_per_day": 5000.0,
            },
        ...
        self.assertAlmostEqual(runtime["strike_price"], 650.0 * 830.0 / 710.0)
        self.assertAlmostEqual(runtime["barrier_in"], 650.0 * 830.0 / 710.0)
        self.assertAlmostEqual(runtime["knock_in_exercise_price"], 650.0 * 830.0 / 710.0)
        self.assertAlmostEqual(
            runtime["params"]["knock_in_exercise_price"],
            650.0 * 830.0 / 710.0,
        )
        event = self.ns["phoenix_acc_fixed_day_event"](
            close=700.0,
            day_index=1,
            total_days=10,
            daily_qty=1.0,
            ...
            kind_value=runtime["kind"],
            entry_price=runtime["entry_price"],
            knock_in_price=runtime["barrier_in"],
            knock_in_exercise_price=runtime["params"]["knock_in_exercise_price"],
            subsidy_per_ton=runtime["subsidy_per_ton"],
            knock_out_price=runtime["knock_out_price"],
            participation_rate=runtime["multiple"],
            knock_in_qty_mode=runtime["params"]["knock_in_qty_mode"],
            knock_out_settlement_mode=runtime["params"]["knock_out_settlement_mode"],
            knock_out_exercise_price=runtime["params"].get("knock_out_exercise_price"),
        )
        self.assertEqual(event["event_type"], "normal_subsidy")
