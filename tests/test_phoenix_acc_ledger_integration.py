...
class PhoenixAccLedgerIntegrationTests(unittest.TestCase):
    ...
    def insert_basic_range_structure(self, structure_id: str, qty: float = 100.0) -> None:
        self.conn.execute(
            """
        ...
        self.conn.execute("INSERT INTO price(dt, underlying, settle) VALUES (?,?,?)", ("2026-04-01", "I2605", 100.0))
        self.conn.commit()
    ...
    def test_call_subsidy_flows_into_group_total_pnl(self) -> None:
        terms = self.call_terms()
        self.insert_structure("ACC_TOTAL", [100.0, 101.0, 102.0], terms)
        _, group_df, _ = self.app.compute_ledgers(self.conn)
        actual = (
            group_df[group_df["underlying"].astype(str) == "ACC_TOTAL.TEST"]
            .sort_values("date")
            .reset_index(drop=True)
        )
        self.assertEqual(len(actual), 3)
        expected_subsidy = terms["daily_qty"] * terms["subsidy_per_ton"] * 3
        self.assertAlmostEqual(float(actual.iloc[-1]["subsidy_pnl"]), expected_subsidy)
        self.assertAlmostEqual(float(actual.iloc[-1]["total_pnl"]), expected_subsidy)

    def test_close2_pnl_is_limited_to_absorbed_position_qty(self) -> None:
        self.insert_basic_range_structure("S_SPLIT", qty=100.0)
        self.conn.executemany(
            ...
            """,
            [
                ("C1", "2026-04-01", "G1", "S_SPLIT", "I2605", "SELL", 60.0, 100.0, 110.0, 600.0, self.app.STRUCT_CLOSE_CATEGORY, 0),
                ("C2", "2026-04-01", "G1", "S_SPLIT", "I2605", "SELL", 50.0, 100.0, 110.0, 500.0, self.app.STRUCT_CLOSE_CATEGORY, 0),
            ],
        )
        self.conn.commit()

        _, group_df, _ = self.app.compute_ledgers(self.conn, as_of_date="2026-04-01")
        row = group_df[group_df["underlying"].astype(str).eq("I2605")].iloc[-1]

        self.assertAlmostEqual(float(row["position_close_pnl"]), 1000.0)
        self.assertAlmostEqual(float(row["close_pnl"]), 1000.0)
        self.assertAlmostEqual(float(row["net_pos_qty"]), 0.0)
